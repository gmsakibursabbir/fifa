    ...
    <section className="py-6 border-b border-white/5">
      ...
      {display.length === 0 ? (
        ...
        <div className="flex gap-3 sm:gap-5 overflow-x-auto pt-3 sm:pt-4 pb-6 sm:pb-8 px-4 sm:px-8 md:px-16 -mx-4 sm:-mx-8 md:-mx-16 hide-scrollbar">
          {display.map((match) => (
            <div key={match.id} className="w-[260px] sm:w-[300px] md:w-[340px] shrink-0">
    ...
    <section className="py-6 border-b border-white/5">
      ...
      <div className="flex gap-3 sm:gap-5 overflow-x-auto pt-3 sm:pt-4 pb-6 sm:pb-8 px-4 sm:px-8 md:px-16 -mx-4 sm:-mx-8 md:-mx-16 hide-scrollbar">
        {channels.slice(0, 8).map((ch, i) => (
          <div key={ch.id} className="w-[240px] sm:w-[260px] md:w-[280px] shrink-0">
      ...
      <div className="flex items-center gap-2 sm:gap-3 overflow-x-auto pb-3 mb-6 sm:mb-8 hide-scrollbar">
        {[
          ...
          <Link
            key={href}
            href={href}
            className="flex items-center gap-1.5 sm:gap-2 px-4 sm:px-6 py-2.5 sm:py-3 rounded-full bg-white/5 border border-white/5 text-white/70 hover:text-white hover:bg-white/10 hover:border-white/10 transition-all font-semibold uppercase tracking-wider text-[11px] sm:text-xs shrink-0"
          >
